...
const AdminBookingsPage: React.FC = () => {
  ...
  const fetchBookings = async () => {
    if (!token) return;
    try {
      const response = await fetch('https://car-service-1aoh.onrender.com/api/admin/bookings', {
        headers: {
  ...
  const confirmBooking = async (id: string) => {
    setProcessingId(id);
    try {
      const response = await fetch(`https://car-service-1aoh.onrender.com/api/admin/bookings/${id}/confirm`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const completeBooking = async (id: string) => {
    setProcessingId(id);
    try {
      const response = await fetch(`https://car-service-1aoh.onrender.com/api/admin/bookings/${id}/complete`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
